...
# Спроба вгадувати «справжній» колір літери за палітрою теж не працює —
# край темної літери на білому ближчий до білого, ніж до чорнила.
DIFF = 120
...
def dense(fp, bp, x, y, W, H, r=3):
    ...
    def diff(px, py):
        if not (0 <= px < W and 0 <= py < H):
            return 0
        f, b = fp[px, py], bp[px, py]
        return abs(f[0] - b[0]) + abs(f[1] - b[1]) + abs(f[2] - b[2])
