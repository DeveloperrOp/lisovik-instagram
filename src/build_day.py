...
def draw_bg(t: dict, cfg: dict, looks: dict, outdir: Path, tok: str,
    ...
    for n in range(1, tries + 1):
        ...
        if ref:
            ...
            exact = {"лісовик", "їжовик", "чага", "кордицепс", "рейші",
                     "ашваганда", "шиїтаке", "гребінчастий", "військовий",
                     "мелений", "мелена", "цілий", "ціла", "капсули"}
            spoiled = []
            for x in lines:
                for w in re.split(r"[\s,.–—-]+", x.lower()):
                    if len(w) < 4 or w in exact:
                        continue
                    near = difflib.get_close_matches(w, exact, 1, 0.72)
                    if near:
